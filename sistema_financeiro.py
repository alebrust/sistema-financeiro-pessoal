# --- ARQUIVO: sistema_financeiro.py (VERSÃO 56 - PAGAMENTO DE FATURA) ---
...
class GerenciadorContas:
    ...
    def pagar_fatura(self, id_fatura: str, id_conta_pagamento: str, data_pagamento: date) -> bool:
        ...
        if not fatura or not isinstance(conta_pagamento, ContaCorrente) or fatura.status == "Paga":
            return False
        
        # Tenta sacar o dinheiro da conta corrente
        if not conta_pagamento.sacar(fatura.valor_total):
            return False # Saldo insuficiente

        # Se o saque foi bem-sucedido:
        fatura.status = "Paga"
        
        # Marca todas as compras daquela fatura como pagas
        for compra in self._compras_cartao:
            if compra.id_fatura == id_fatura:
                compra.paga = True
        
        # Registra a transação de despesa na conta corrente
    ...
    def registrar_compra_cartao(self, id_cartao: str, descricao: str, valor_total: float, data_compra: date, categoria: str, num_parcelas: int = 1, observacao: str = ""):
        ...
        for i in range(num_parcelas):
            ...
            data_fatura_parcela = date(ano_fatura, mes_fatura, 1) + relativedelta(months=i)
            ...
            nova_compra = CompraCartao(id_cartao=id_cartao, descricao=desc_parcela, valor=valor_parcela, data_compra=data_vencimento_final, categoria=categoria, total_parcelas=num_parcelas, parcela_atual=i + 1, id_compra_original=id_compra_original, observacao=observacao)
